...
const quizQuestions: Question[] = [
  {
    ...
    correct: 1,
    explanation:
      "Systems with a TXV maintain a constant superheat at the evaporator outlet. Therefore, liquid line subcooling is the primary indicator used to verify proper refrigerant charge.",
  },
  {
    q: "If a system has low suction pressure and high superheat, what is the most likely issue?",
    options: [
      "Blocked condenser fan motor",
      "Overcharged refrigerant",
      "Undercharged refrigerant or restricted expansion valve",
      "Dirty indoor air filter",
    ],
    correct: 2,
    explanation:
      "Low refrigerant charge or a restricted metering device reduces refrigerant flow to the evaporator, resulting in low suction pressure and elevated suction line superheat.",
  },
  {
    q: "What does a high vacuum level of 2000 microns after evacuation indicate?",
    options: [
      "A dry, moisture-free system",
      "Presence of moisture or a vacuum leak",
      "A fully charged refrigeration loop",
      "An optimal deep vacuum",
    ],
    correct: 1,
    explanation:
      "An industry-standard deep vacuum is 500 microns or below. A reading that rises and stays at 2000 microns indicates trapped moisture or a small leak in the piping.",
  },
  {
    ...
    correct: 1,
    explanation:
      "R32 has a GWP of 675 (compared to 2088 for R410A) and is classified under safety standard ASHRAE 34 as A2L (mildly flammable).",
  },
  {
    ...
    options: [
      ...
      "It drops, eventually leading to evaporator coil freezing",
      "The discharge pressure will double instantly",
    ],
    correct: 2,
    explanation:
      "Without air blowing over the evaporator coil, heat transfer stops. The temperature and pressure of the refrigerant drop, causing the coil to freeze.",
  },
  {
    ...
    correct: 1,
    explanation:
      "Compression Ratio = (Discharge + 14.7) / (Suction + 14.7) = (350 + 14.7) / (70 + 14.7) = 364.7 / 84.7 = 4.3:1. (Always use absolute pressures).",
  },
  {
    ...
    correct: 2,
    explanation:
      "POE oil is the standard lubricant for HFC refrigerants like R134a and R410A because of its compatibility. However, POE is highly hygroscopic and absorbs moisture rapidly.",
  },
  {
    q: "In a commercial walk-in freezer, what is the typical function of the defrost termination thermostat?",
    options: [
      "To start the compressor when the temperature rises",
      "To terminate defrost cycle based on coil temperature",
      "To sound an alarm if the door is left open",
      "To control the liquid line solenoid valve",
    ],
    correct: 1,
    explanation:
      "Defrost heaters melt frost on the evaporator coil. The termination thermostat senses when the coil has warmed up enough to ensure all frost has melted, switching the system back to refrigeration mode.",
  },
  {
    ...
    correct: 2,
    explanation:
      "A suction accumulator is a reservoir placed before the compressor suction port designed to catch liquid refrigerant, allowing only vapor to enter the compressor.",
  },
  {
    q: "What causes non-condensable gases to accumulate inside an air conditioning system?",
    options: [
      "Refrigerant decomposition from high temperatures",
      "Charging the system with liquid instead of vapor",
      "Failure to pull a proper vacuum before charging",
      "A faulty thermal expansion valve bulb",
    ],
    correct: 2,
    explanation:
      "Failing to evacuate air and moisture from the lines before charging traps nitrogen, oxygen, and moisture in the condenser, raising head pressure and energy costs.",
  },
];
...
export function HvacQuizView() {
  ...
  if (quizFinished) {
    return (
      <InteractiveCard
        title="HVAC/R Technician Quiz"
        desc="Test your refrigeration cycle, diagnostics, and standards knowledge."
      >
        <div className="text-center py-6 space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            You scored <strong className="text-primary text-lg">{score}</strong> out of{" "}
            <strong className="text-foreground text-lg">{quizQuestions.length}</strong>.
          </p>
          ...
          <div className="bg-card/25 p-4 rounded-xl border border-border max-w-md mx-auto text-xs space-y-2">
            ...
            {score >= 9 ? (
              <p className="text-emerald-400 font-semibold">
                Master Level! You have outstanding diagnostic skills and technical understanding.
              </p>
            ) : score >= 6 ? (
              <p className="text-yellow-400 font-semibold">
                Competent Technician! A solid grasp, but some study of thermodynamics is
                recommended.
              </p>
            ) : (
              <p className="text-rose-400 font-semibold">
                Apprentice Level! Keep practicing and reviewing standard HVAC manuals.
              </p>
            )}
          </div>
          ...
          <button
            onClick={resetQuiz}
            className="px-5 py-2 rounded-xl bg-primary text-primary-foreground font-semibold text-xs hover:bg-primary/95 transition inline-flex items-center gap-1.5 mt-4"
          >
            ...
            {q.options.map((opt, i) => {
              ...
              return (
                ...
                >
                  ...
                  {submitted && selectedOpt === i && i !== q.correct && (
                    <AlertTriangle className="h-4 w-4 text-rose-400 shrink-0 ml-2" />
                  )}
                </button>
              );
            })}
...
const wizardTree: Record<string, WizardNode> = {
  start: {
    ...
    choices: [
      {
        text: "System is not cooling at all (Warm air blowing / Compressor off)",
        nextNode: "not_cooling",
      },
      {
        text: "System is cooling, but not enough (Insufficient capacity / Constant run)",
  ...
  not_cooling: {
    ...
    choices: [
      {
        text: "Yes, fan is spinning but compressor feels off/humming",
        nextNode: "fan_runs_comp_off",
      },
  ...
  fan_runs_comp_off: {
    question:
      "A humming compressor that doesn't start usually points to electrical motor assist issues.",
    choices: [],
    resolution:
      "Recommendation: Inspect the Dual Run Capacitor. A bulged, failed capacitor is the #1 reason a compressor fails to start. If the capacitor measures fine, the compressor is likely locked up mechanically or has tripped its internal overload.",
  },
  outdoor_dead: {
    question: "Is the indoor thermostat sending a cooling signal (contactor pulled in)?",
    choices: [
      {
        text: "Yes, contactor button is pushed in on the outdoor unit",
        nextNode: "contactor_engaged",
      },
      {
        text: "No, outdoor unit is not receiving 24V control signal",
        nextNode: "no_control_voltage",
      },
    ],
  },
  contactor_engaged: {
    ...
    choices: [],
    resolution:
      "Recommendation: Check the outdoor disconnect switch box and the main circuit breaker. You likely have a blown cartridge fuse, a tripped breaker, or burnt wiring at the contactor terminals.",
  },
  no_control_voltage: {
    question: "No 24V signal is coming from the thermostat or control board.",
    choices: [],
    resolution:
      "Recommendation: Check the thermostat settings, the safety switches (like float switches in drain pans), and the low-voltage fuse on the indoor furnace/air handler control board. Tripped condensate overflow switches are a common cause.",
  },
  insufficient_cooling: {
    question:
      "Measure the temperature difference (Delta T) between the supply air and return air grills:",
    choices: [
      { text: "Delta T is less than 15°F (Weak cooling)", nextNode: "weak_delta_t" },
      {
        text: "Delta T is normal (18°F to 22°F) but indoor stays warm",
        nextNode: "normal_delta_t_warm",
      },
    ],
  },
  weak_delta_t: {
    question:
      "Are the refrigeration pipes leading to the outdoor unit showing any unusual indicators?",
    choices: [
  ...
  suction_line_frost: {
    ...
    choices: [],
    resolution:
      "Recommendation: Airflow issue. Check for a clogged air filter, closed register dampers, dirty evaporator coil fins, or a slipping fan belt. If airflow is clear, the system has a low refrigerant charge (leak).",
  },
  warm_suction: {
    question: "Warm suction suggests lack of refrigerant or inactive expansion valve.",
    choices: [],
    resolution:
      "Recommendation: Check refrigerant pressures. This indicates a leak leading to an undercharged loop, or a stuck-closed TXV restricting refrigerant flow.",
  },
  normal_delta_t_warm: {
    question:
      "Your AC is generating standard cooling output, but cannot keep up with the structural load.",
    choices: [],
    resolution:
      "Recommendation: Check for extreme outdoor temperatures, lack of building insulation, open doors/windows, or an undersized AC unit for the space. Jet clean the outdoor condenser coil as high head pressure reduces operational capacity.",
  },
  short_cycling: {
  ...
  low_pressure_trip: {
    ...
    choices: [],
    resolution:
      "Recommendation: Verify refrigerant charge. The low pressure switch cuts the compressor to protect it from running dry due to a refrigerant leak. Also check for a completely blocked evaporator filter.",
  },
  high_pressure_trip: {
    question: "High pressure cuts out.",
    choices: [],
    resolution:
      "Recommendation: Condenser heat dissipation failure. Wash dirt out of the condenser coils. Ensure the outdoor fan is spinning at full speed. Overcharging the system will also trip the high-pressure switch.",
  },
  ice_formation: {
    question: "Is the air filter clean and the indoor blower motor spinning at full speed?",
    choices: [
      {
        text: "Yes, air filter is clean and blower is running fine",
        nextNode: "suction_line_frost",
      },
  ...
  fix_airflow: {
    ...
    choices: [],
    resolution:
      "Recommendation: Replace the air filter immediately and clean the indoor fan blower. Let the coil defrost completely (run FAN ONLY mode) before starting cooling again.",
  },
};
...
export function TroubleshootingWizardView() {
  ...
  return (
    <InteractiveCard
      title="Troubleshooting Diagnostic Wizard"
      desc="Interactive decision tree to diagnose common faults in air conditioners and commercial coolers."
    >
      <div className="space-y-6">
        <div className="bg-card/25 border border-border/80 p-5 rounded-2xl relative">
          <div className="absolute top-3 right-3 text-[10px] text-muted-foreground uppercase font-semibold">
            Diagnostic Engine
          </div>

          <div className="space-y-4">
            {!node.resolution && (
              <h3 className="text-base font-bold text-foreground pr-10">{node.question}</h3>
            )}
            ...
            {node.resolution ? (
              <div className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Disclaimer: High-voltage diagnostics and refrigerant line servicing must only be
                  performed by certified technicians. Call Prime Cool Solutions for on-site support.
                </p>
              </div>
            ) : (
              <div className="space-y-2">
        ...
        <div className="flex justify-between items-center">
          {history.length > 0 ? (
            <button
              onClick={handleBack}
              className="px-4 py-2 rounded-lg border border-border hover:bg-card/50 text-xs font-semibold text-muted-foreground transition"
            >
              Back
            </button>
          ) : (
          ...
          <button
            onClick={handleReset}
            className="px-4 py-2 rounded-lg border border-border/60 hover:bg-card/50 text-xs font-semibold text-primary transition flex items-center gap-1"
          >
  ...
  if (app === "ac") {
    if (gwpLimit < 700) {
      if (flammableAllowed) {
        selection = "R32";
        why =
          "Excellent thermal performance and lower charge size. Standard replacement for R410A with moderate A2L flammability.";
      } else {
        selection = "R410A";
        why =
          "Non-flammable safety compliance limits low-GWP options. R410A is a safe A1 selection despite its GWP of 2088.";
      }
    } else {
      selection = "R410A";
      why =
        "Widely available, high-pressure design, zero flammability hazard. Best compatibility for residential and light commercial splits.";
    }
  } else if (app === "med-temp") {
    if (gwpLimit < 2000) {
      selection = "R134a";
      why =
        "Low operating pressures and high critical temperature. Perfect for commercial display cases and medium-temperature walk-ins.";
    } else {
      selection = "R407C / R404A";
      why =
        "Widely supported for mid-temp refrigeration, though R404A has a high GWP of 3922 which is subject to phase-downs.";
    }
  } else {
    // low-temp
    if (gwpLimit < 2500) {
      ...
      why =
        "Lower-GWP HFO blends designed as eco-friendly drop-ins to replace R404A in freezer rooms.";
    } else {
      selection = "R404A";
      why =
        "Classic standard for deep freezing (-20°C to -30°C). Provides outstanding envelope reliability, but restricted in new installs due to GWP.";
    }
  }

  return (
    <InteractiveCard
      title="Refrigerant Selection Assistant"
      desc="Find the optimal refrigerant choice based on application temperature guidelines, flammability constraints, and GWP limits."
    >
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-muted-foreground font-semibold block">
              Maximum Allowable GWP Index
            </label>
            <div className="flex gap-4 text-xs">
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input
                  type="radio"
                  checked={gwpLimit < 1000}
                  onChange={() => setGwpLimit(700)}
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-muted-foreground font-semibold block">
              Flammable (A2L/A3) Refrigerants Allowed?
            </label>
            <div className="flex gap-4 text-xs">
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input
                  type="radio"
                  checked={flammableAllowed}
                  onChange={() => setFlammableAllowed(true)}
                  className="accent-primary"
                />
              ...
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input
                  type="radio"
                  checked={!flammableAllowed}
                  onChange={() => setFlammableAllowed(false)}
                  className="accent-primary"
                />
...
export function CostEstimatorView() {
  ...
  return (
    <InteractiveCard
      title="AC Installation Cost Estimator"
      desc="Calculate the estimated installation labor and material costs for air conditioners in Pune & Mumbai districts."
    >
      <div className="grid md:grid-cols-2 gap-6 items-start">
        <div className="space-y-4">
          <div className="space-y-1.5">
        ...
        <div className="bg-background/40 border border-border p-5 rounded-2xl space-y-4">
          ...
          <div className="space-y-1 pt-2 border-t border-border/40 text-xs">
            ...
            <div className="pt-3 font-semibold text-foreground leading-normal">
              Note: Estimates exclude core-cutting through concrete structural walls and scaffold
              hoisting charges if installing higher than second floors.
            </div>
          </div>
        </div>
      </div>
...
export function PmChecklistGeneratorView() {
  ...
  return (
    <InteractiveCard
      title="Preventive Maintenance Checklist Generator"
      desc="Generate customized mechanical PM checklist sheets for home air conditioners, commercial chillers, or cold rooms."
    >
      <div className="space-y-6">
            ...
            <div className="flex justify-between items-start pb-4 border-b border-border/40">
              <div>
                <h3 className="text-lg font-bold text-foreground capitalize">
                  {eqType} PM Work Checklist
                </h3>
                <p className="text-[10px] text-muted-foreground mt-0.5">
                  Prime Cool Solutions Quality Compliance Form
                </p>
              </div>
              <span className="text-[10px] bg-primary/20 text-primary border border-primary/30 px-2 py-0.5 rounded-full uppercase font-semibold">
                Standard PM Form
              </span>
            </div>
...
export function RoiCalculatorView() {
  ...
  return (
    <InteractiveCard
      title="AC Efficiency ROI & Payback Calculator"
      desc="Calculate the estimated monthly energy bill savings and amortization period when upgrading to a high-efficiency inverter AC."
    >
      <div className="grid md:grid-cols-2 gap-6 items-start">
        <div className="space-y-4">
          <NumericInput
          ...
          />
          <NumericInput
            label="Daily Hours of Operation"
            value={hours}
            onChange={setHours}
            unit="Hours/Day"
            min={2}
          ...
          />
          <NumericInput
            label="Cost of New AC System"
            value={newAcCost}
            onChange={setNewAcCost}
            unit="₹"
            min={25000}
            max={120000}
            step={2000}
          ...
          <div className="space-y-1.5 pt-3 border-t border-border/40 text-xs">
            ...
            <div className="flex justify-between py-1 border-b border-border/20 text-muted-foreground">
              ...
              <span className="font-mono text-emerald-400 font-bold">
                ₹{Math.round(annualSavings * 5).toLocaleString()}
              </span>
            </div>
            <div className="pt-2 text-xs text-muted-foreground leading-relaxed">
              Note: ROI calculations are based on average cooling load cycles. High-efficiency
              systems also increase building grid safety by drawing fewer startup starting amperes.
            </div>
          </div>
        </div>
      </div>
